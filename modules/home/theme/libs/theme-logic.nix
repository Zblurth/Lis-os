...
in
{
  home.packages = [
    ...
    (pkgs.writeShellScriptBin "theme-engine" ''
      ...
      # FIX: Added AGS Colors
      process "ags-colors.css" "$HOME/.cache/wal/ags-colors.css"
    ...
    (pkgs.writeShellScriptBin "theme-manager" ''
      ...
      # FIX: Only start swww if not running to avoid error spam
      ...
      if pgrep -x ags > /dev/null; then
          # We use pkill because AGS v1 hot reload can be finicky with config changes
          pkill ags
          ags &
      fi
